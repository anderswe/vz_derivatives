...
# load original object preprocessed by liam, params in file name.
...
mks <- FindAllMarkers(so, only.pos = T) %>% dplyr::mutate(unique = pct.1 - pct.2, geneid = rownames(.))
...
so@assays$RNA@counts %>% qs::qsave("out/cs20s_annot_rna_counts.qs")
so@meta.data %>% tibble::rownames_to_column("sample_barcode") %>% readr::write_csv("out/metadata_for_cs20s_annot_rna_counts.csv")
...
# all markers
Idents(so) <- so$celltype_fine
all_mks <- FindAllMarkers(so, only.pos = T) %>% dplyr::mutate(unique = pct.1 - pct.2, geneid = rownames(.))
data.table::fwrite(all_mks, "out/all_markers_cs20s.csv")
...
nemks %>% writexl::write_xlsx(glue::glue("~/sandbox/ne_markers.xlsx"))
...
nemks2 <- FindMarkers(so, ident.1 = "NE", ident.2 = "VZ & RL") %>% dplyr::mutate(gene = rownames(.))
...
do_volcano(nemks2, goi = c("PTPRZ1", "ZIC1", "ERBB4", "ATXN1", "WNT7B", "DCC", "NFIA", "LRP2"), label1 = "NE", label2 = "VZ & RL")
dev.off()

pdf("out/volcano_ne_vs_rl&vz_expanded.pdf", w = 10, h = 9)
do_volcano(nemks2, label1 = "NE", label2 = "VZ & RL")
dev.off()
